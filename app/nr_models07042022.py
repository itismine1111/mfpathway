...
class MakeModel(EmbeddedDocument):
    ...
    def get_json(self, children=False):
        return{
            ...
            "created_at":self.created_at,
            "updated_at":self.updated_at,
            "products":self.list_of_products_by_model(str(self._id)) if children else None
        }

    @staticmethod
    def list_of_products_by_model(model_id):
        products=[]
        for product in AutopartProduct.retrieve_all_products():
            if product["model_id"] == model_id:
...
class Brand(Document):
    ...
    def retrieve_all_brand(cls, limit=None,skip=None, children=False,status='D'):
        with connect_to_db():
            if status=='A':
                return [item.get_json(children,status=status) for item in cls.objects.filter(status='A').skip(skip).limit(limit)]
            else:
                return [item.get_json(children,status=status) for item in cls.objects.filter(status__ne='D').skip(skip).limit(limit)]
...
class ImageUploads(Document):
    ...
    def retrieve_all_images(cls, parent_id, limit=None,skip=None ):
        with connect_to_db():
            return [item.get_json() for item in cls.objects.filter(Q(status='A')&Q(parent_id=parent_id)).skip(skip).limit(limit)]
...
class AutopartProduct(Document):
    ...
    shipping_weight=FloatField(required=True)
    ...
    @classmethod
    def retrieve_all_products(cls, limit=None,skip=None,categories=None, models=None,search_text=None):
