...
class AutopartProduct(Document):
    ...
    location_city=IntField(required=True)
    location_province=IntField(required=True)
    location_country=IntField(required=True)
    condition=StringField(required=True)
    warranty=StringField(required=True)
    shipping=BooleanField()
    standard_shipping_cost=StringField(required=True)
    ...
    regular_price = StringField(requried=True)
    ...
    quantity= StringField(requried=True)
    ...
    @classmethod
    def find_by_brand_id(cls,brand_id):
        with connect_to_db():
             return [item.get_json() for item in cls.objects.filter(brand_id=brand_id)]
