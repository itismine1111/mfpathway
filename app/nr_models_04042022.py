...
class AutopartProduct(Document):
    ...
    def get_json(self):
        ...
        return{
            ...
            "images":ImageUploads.retrieve_all_images(str(self._id))
        }
...
class ImageUploads(Document):
    ...
    def retrieve_all_images(cls, parent_id, limit=None,skip=None, ):
        with connect_to_db():
            return [item.get_json() for item in cls.objects.filter(Q(status__ne='D')&Q(parent_id=parent_id)).skip(skip).limit(limit)]
