...
class AutopartProduct(Document):
    ...
    @classmethod
    def retrieve_all_products_list(cls, limit=None, skip=None,subcategory_slug=None,subcategory=None):
        all_product = list()
        filter = Q(status__ne='D')
        if subcategory_slug:
...
class Store_Product(Document):
    _id = ObjectIdField()
    user_id = StringField(required=True)
    color_id=StringField(required=True)
    product_id=StringField(required=True)
    regular_price=FloatField (required=True) 
    sale_price=FloatField(required=True)
    stock=BooleanField(default=False)
    ...
    @classmethod
    def find_by_color_id(cls, color_id,product_id):
        with connect_to_db():
            try:
                return cls.objects.filter(color_id=color_id,product_id=product_id)
            except DoesNotExist:
                return None
